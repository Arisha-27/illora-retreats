...
class Welcome_IloraRetreatsConciergeBot:
    ...
    **Optimized with background data refreshing to ensure fast response times.**
    ...
    def __init__(self):
        ...
        # SECURITY UPDATE: Load API Key from Environment Variable ONLY
        self.llm_api_key = os.environ.get("GOOGLE_API_KEY")
        
        if not self.llm_api_key:
            # Fallback to Config object if env var not set, but ensure Config is not committed to git
            self.llm_api_key = getattr(Config, "GOOGLE_API_KEY", None)

        if not self.llm_api_key:
            logger.error("CRITICAL: Google API Key is missing! Set GOOGLE_API_KEY in Environment Variables.")
            # In production, you might want to raise an error here to prevent starting without auth
        ...
        self.llm_model_name = "gemini-1.5-pro" # Updated to standard model name

        # Sheet configuration
        # SECURITY UPDATE: Load Sheet URL from Environment Variable
        self.sheet_api = os.environ.get("GSHEET_WEBAPP_URL", getattr(Config, "GSHEET_WEBAPP_URL", None))
    ...
    def _fetch_sheet_data(self, sheet_name: str) -> List[Dict[str, Any]]:
        if not self.sheet_api:
            logger.error("Sheet API URL is missing.")
            return []
    ...
    def _call_llm_gemini(self, prompt: str, max_retries: int = 3) -> str:
        """
        Executes the prompt using Google Gemini.
        """
